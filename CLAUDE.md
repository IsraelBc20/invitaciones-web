...
- **Rutas de clientes** (14 jul 2026): `/oliva/israel-y-marisol` y `/playa/israel-y-marisol` (+ sus `/galeria`) son las invitaciones REALES — sin prop `demo`, con `galeriaHref` apuntando a su propia galería y metadata "Israel & Marisol — 12 de Septiembre 2026". Cómo crear un cliente nuevo: ver `README-CLIENTE.md`
...
- **Fotos de los novios** (14 jul 2026): el componente `Photo` carga `/photos/<archivo>` con nombres fijos (9 huecos: `foto-pareja-hero`, `foto-colegio`, `foto-juntos`, `foto-momento`, `foto-pareja-circular`, `foto-full-bleed`, `foto-collage-grande`, `foto-collage-chica-1/2`; tabla con proporciones en `README-CLIENTE.md` y `public/photos/README.txt`); mientras el archivo no exista muestra un placeholder oliva con ícono de cámara y el nombre esperado — basta copiar las fotos reales a `public/photos/` para que se reemplacen solas.
...
- [ ] Copiar las 9 fotos reales de los novios a `public/photos/` con los nombres exactos (ver `public/photos/README.txt`); los placeholders de `/oliva` se reemplazan solos
